...
%
classdef SingleLayer_H_a < ops.Operators
	% Class for single layer potential operator for Helmholtz equation
	
	methods
		function obj = SingleLayer_H_a(k, alpha, D1, type1, step1, D2, type2, step2)
			...
			if isequal(D1,D2)
				obj.Kmat = ops.SingleLayer_H.make_kernel_matrix(k, alpha, D1.points, D1.sigma);
			else
				obj.Kmat = ops.SingleLayer_H.make_kernel_matrix(k, alpha, D1.points, D1.sigma, D2.points);
			end
		end
	...
	methods(Static)
		function K = make_kernel_matrix(k, alpha, D, sigma_D, E)
			% Kernel matrix of the single layer potential L^2(\p D) -> L^2(\p E).
			green = @(k,x,y) tools.GBiPeriodic(k, x, y, 1, 1, alpha); 
			if nargin == 4
                N = size(D,2);
