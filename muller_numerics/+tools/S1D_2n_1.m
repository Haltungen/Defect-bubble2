function sum=S1D_2n_1(n,k,alp,L,Num_m)
...
sum=sum+2*(-1)^n*b0*L*n/pi^2*(L*k/(4*pi))^(2*n-1)*zeta(2*n+1);
...
for m=0:n-1
    ...
    sum_temp2=sum_temp2+(-1)^m*2^(2*m)*factorial(n+m-1)/(factorial(2*m+1)*factorial(n-m-1))*(2*pi/k/L)^(2*m+1)*tools.bernoulli_poly2(2*m+1, alp(1)*L/(2*pi));
