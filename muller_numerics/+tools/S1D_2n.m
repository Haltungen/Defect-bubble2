function sum=S1D_2n(n,k,alp,L,Num_m)
...
for m=1:n
    ...
    sum_temp2=sum_temp2+(-1)^m*2^(2*m)*factorial(n+m-1)/(factorial(2*m)*factorial(n-m))*(2*pi/k/L)^(2*m)*tools.bernoulli_poly2(2*m, alp(1)*L/(2*pi));
